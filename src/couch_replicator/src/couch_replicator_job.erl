...
do_init(Job, #{} = JobData) ->
    ...
    {Job2, JobData2, Owner} = couch_jobs_fdb:tx(JobsTx, fun(JTx) ->
        {Job1, JobData1} = init_job_data(JTx, Job, JobData, RepId, BaseId),
...
init_job_data(_, Job, #{?REP_ID := RepId} = JobData, RepId, _) ->
    {Job, JobData};
